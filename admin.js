  ...
  search: {
    ...
    fields: [
      ...
      [
        "match_mode",
        "Correspondência",
        "select:exact|contains|starts_with|approximate",
      ],
...
async function init() {
  if (document.body.dataset.page === "login") return login();
  bindGlobal();
  await authorize();
}
async function login() {
  loginForm.onsubmit = async (e) => {
    ...
    const { data, error } = await A.client.auth.signInWithPassword({
...
function bindGlobal() {
  ...
  document
    ...
    .forEach((x) => (x.onclick = closeEditor));
}
async function authorize() {
  try {
    ...
    A.user = session.user;
    const { data, error: ae } = await A.client
      .from("blog_admins")
      .select("*")
...
function buildNav() {
  ...
  nav.innerHTML = sections
    .map((s) => `<button data-s="${s}">${labels[s]}</button>`)
    .join("");
  nav
...
async function overview() {
  ...
  content.innerHTML = `<section class="hero-card"><p class="tag">ARQUIVO CONECTADO</p><h2>O SISTEMA ESTÁ RESPONDENDO.</h2><p>Todos os módulos principais estão prontos para configuração.</p></section><section class="stats">${names.map((n, i) => `<article class="stat"><small>${n[1]}</small><b>${counts[i]}</b><span>${n[0]}</span></article>`).join("")}</section><div class="help"><b>Daiana-56</b><p>O código inicial do chat Algo já é criado pelo SQL. Configure o chat e suas regras no módulo CHATS.</p></div>`;
}
async function listView(s) {
  ...
  A.cache[s] = data || [];
  content.innerHTML = `<div class="toolbar"><input id="filter" placeholder="Pesquisar nesta lista"><button id="newItem">+ NOVO</button></div><div id="list" class="data-list"></div>`;
    ...
    list.querySelectorAll("[data-edit]").forEach(
      (b) =>
        (b.onclick = () =>
          openEditor(
            s,
            A.cache[s].find((x) => x.id === b.dataset.edit),
          )),
    );
    list
      .querySelectorAll("[data-dup]")
      .forEach((b) => (b.onclick = () => duplicate(s, b.dataset.dup)));
    list
      .querySelectorAll("[data-del]")
      .forEach((b) => (b.onclick = () => remove(s, b.dataset.del)));
    list
      .querySelectorAll("[data-status]")
      .forEach((b) => (b.onclick = () => toggleStatus(s, b.dataset.status)));
  }
}
async function openEditor(s, item) {
  ...
  editorForm.innerHTML =
    d.fields.map((f) => fieldHTML(f, item || {}, options)).join("") +
...
async function preloadOptions() {
  ...
  for (const [k, t, l] of [
    ...
    const { data } = await A.client.from(t).select(`id,${l}`);
    out[k] = (data || []).map((x) => ({ id: x.id, label: x[l] }));
  }
  return out;
}
function fieldHTML(f, item, o) {
  ...
  if (t.startsWith("select:")) {
    ...
    const opts = spec.includes("|")
      ? spec.split("|").map((x) => ({ id: x, label: x }))
      : o[spec] || [];
...
async function saveEditor(e) {
  ...
  for (const [n, , t] of d.fields) {
    ...
    if (t === "checkbox") payload[n] = !!el?.checked;
      ...
      payload[n] = fd.get(n) === "" ? null : Number(fd.get(n));
  ...
  if (
    ...
    payload.slug = slug(payload[d.label]);
  if (["posts", "characters", "resources"].includes(s) && !payload.status)
  ...
  const q = id
    ? A.client.from(d.table).update(payload).eq("id", id)
    : A.client.from(d.table).insert(payload).select().single();
  const { data, error } = await q;
  if (error) return alert(error.message);
  const saved = id ? { id } : data;
  ...
  x.status =
    x.status === "active"
      ...
      : x.status === "published"
        ? "draft"
        : x.status;
  const { error } = await A.client.from(d.table).insert(x);
  if (error) alert(error.message);
  else show(s);
}
async function remove(s, id) {
  if (!confirm("Excluir definitivamente?")) return;
  const { error } = await A.client.from(defs[s].table).delete().eq("id", id);
  if (error) alert(error.message);
  else show(s);
}
...
async function mediaView() {
  ...
  content
    ...
    .forEach(
      (b) => (b.onclick = () => navigator.clipboard.writeText(b.dataset.copy)),
    );
  content
    .querySelectorAll("[data-mdel]")
    .forEach(
...
function renderSteps() {
  ...
  stepList.querySelectorAll("[data-step]").forEach(
    (el) =>
      (el.oninput = () => {
        stepData[el.dataset.step][el.dataset.k] =
          el.type === "number" ? Number(el.value) : el.value;
      }),
  );
  stepList.querySelectorAll("[data-stepdel]").forEach(
    (b) =>
      (b.onclick = () => {
...
function renderRules() {
  ...
  ruleList.innerHTML = ruleData
    ...
    .join("");
  ruleList.querySelectorAll("[data-rule]").forEach(
    (el) =>
      (el.oninput = () => {
  ...
  ruleList.querySelectorAll("[data-ruledel]").forEach(
    (b) =>
      (b.onclick = () => {
...
async function saveRules(chatId) {
  const old = await A.client
    .from("blog_chat_rules")
    .select("id")
    .eq("chat_id", chatId);
  if (old.data?.length)
    await A.client.from("blog_chat_rules").delete().eq("chat_id", chatId);
  if (ruleData.length)
    await A.client.from("blog_chat_rules").insert(
      ruleData.map((r) => {
        const x = { ...r, chat_id: chatId };
        delete x.id;
        delete x.created_at;
        delete x.updated_at;
        return x;
